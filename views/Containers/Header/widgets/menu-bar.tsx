...
const MenuBar1: NextPage<MenuBarState> = ({ menuData = data }) => {
  ...
  return (
    <>
      ...
      <li className="mega home">
        <a href="features/portfolio/mesonary-grid-4"
          className="dark-menu-item has-submenu"
          >
          Features
        </a>
          ...
          <li key={i} className={` ${menuItem.megaMenu ? "mega" : ""}`}>
            {(menuItem.type === "sub" || menuItem.megaMenu) && menuItem.children && (
              <a
                ...
                {t(menuItem.title)}
                <span className={`${menuItem.title==="product"} sub-arrow ${(path === "/Layouts/layout3" || mobileSize) && (isOpen === menuItem.title ? "minus" : "plus")}`}></span>
              </a>
            )}
